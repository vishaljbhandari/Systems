...
pre_skip=1
if [ $# -eq 1 ] ; then
	echo -e "[ERROR] Invalid Usage : source "$SCRIPT_NAME" [--pre-skip]"
    pre_skip=0;
fi
function finish {
...
RUN_DETAIL_FILE=$CURR_DIR"/cases/.runDetails"
if [ ! -f $RUN_DETAIL_FILE ] ; then
	echo -e "[ERROR] Missing Run Details File "$RUN_DETAIL_FILE
	exit 995
fi
sed -i '/^$/d' $RUN_DETAIL_FILE > $RUN_DETAIL_FILE.tmp
grep -v '^ *$' $RUN_DETAIL_FILE.tmp | uniq > $RUN_DETAIL_FILE;
rm $RUN_DETAIL_FILE.tmp
echo -e "[00010] Run Details File : "$RUN_DETAIL_FILE
