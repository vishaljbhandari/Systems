...
sub Main(){
	...
	$NumArgs = $#ARGV + 3;
	if ($NumArgs != 1) {
		print "[Error] Usage: $ScriptName stat_file TO_DATE FROM_DATE\n";
		exit 999;
	}
	...
	print ("[ Info] From Time $from_date\n");
	print ("[ Info] To Time $to_date\n");
	...
	if (! -e $RunningConf{-CURRENT_DIR}.'/conf/inspector.conf') {
		print "[ERROR] INVALID/Missing CONF File DIRECTORY, ".$RunningConf{-CURRENT_DIR}.'/conf/inspector.conf\n';
		exit;
	}
