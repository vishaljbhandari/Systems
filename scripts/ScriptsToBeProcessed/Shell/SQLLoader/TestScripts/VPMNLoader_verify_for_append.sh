...
countoffilerecords=`cat $SampleData | wc -l`
doublecountoffilerecords=`expr $countoffilerecords + $countoffilerecords - 2`
if [ $doublecountoffilerecords -eq 0 ]
then
...
if [ $countofdbrecords -ne $doublecountoffilerecords ]
then
    echo "All Data records are not appended to database"
    exit 6
fi
rm vpmnloader.tmp
