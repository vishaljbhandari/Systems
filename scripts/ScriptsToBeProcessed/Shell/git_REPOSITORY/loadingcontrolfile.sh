...
sed -e 's/\$LOADTYPE/'$LOADTYPE'/g' $var/bin/loadingcontrolfile.ctl > $var/tmp/loadingcontrolfile.ctl.tmp
CONNECT_TO_SQLLDR_START
                control=$var/tmp/loadingcontrolfile.ctl.tmp \
                log=temp.log \
                data=$FilePath \
                bad=$var/tmp/loadingcontrolfile.bad \
                discard=$var/tmp/loadingcontrolfile.discard \
                skip=1 \
		silent="header" \
		rows=64
...
cat temp.log >> $var/LOG/loadingcontrolfile.log
rm temp.log
