...
echo "Started Deleting duplicates at `date`" >>delete_duplicate_entries_cdr.log
sqlplus $DB_USER/$DB_PASSWORD <<EOF >>delete_duplicate_entries_sql_cdr.log

create table cdr_dup_deletion_counter (partition_name varchar2(50), batch_count number(10), batch_time date);
...
if [ $? -ne 0 ]
then
    echo "Failed to delete duplicate entries from cdr `date`" >>delete_duplicate_entries_cdr.log
else
    echo "Deleted duplicate entries from cdr `date`" >>delete_duplicate_entries_cdr.log
fi
