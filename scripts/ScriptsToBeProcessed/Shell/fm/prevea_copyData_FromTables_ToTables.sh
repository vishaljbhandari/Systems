...
copyFromDB2ExistingBackupTables()
{
    ...
    sqlplus -s prevea/prevea << EOF > /dev/null
        ...
        SPOOL $WATIR_SERVER_HOME/Scripts/Fixture/tmp/createDBbackupToExisting;
        SET FEEDBACK ON ;
        SET SERVEROUTPUT ON ;
        declare
...
copyFromBackup2DB()
{
    ...
    sqlplus -s prevea/prevea << EOF > /dev/null
        whenever sqlerror exit 1 ;
        whenever oserror exit 1 ;
        SPOOL $WATIR_SERVER_HOME/Scripts/Fixture/tmp/createDBbackupToExistingDelete;
        SET FEEDBACK ON ;
        SET SERVEROUTPUT ON ;
        declare
...
EOF
    ...
    sqlplus -s prevea/prevea << EOF > /dev/null
        whenever sqlerror exit 1 ;
        whenever oserror exit 1 ;
        SPOOL $WATIR_SERVER_HOME/Scripts/Fixture/tmp/createDBbackupToExistingInsert;
        SET FEEDBACK ON ;
        SET SERVEROUTPUT ON ;
        declare
        cursor c1 is select * from at_table order by id;
...
createBackupTables()
{
    ...
    sqlplus -s prevea/prevea << EOF > /dev/null
        whenever sqlerror exit 1 ;
        whenever oserror exit 1 ;
        SPOOL $WATIR_SERVER_HOME/Scripts/Fixture/tmp/createDBbackup ;
        SET FEEDBACK ON ;
        SET SERVEROUTPUT ON ;
        declare
        cursor c1 is select * from at_table order by id;
...
{
	backUpTablesExist=`sqlplus -s prevea/prevea << EOF 2> /dev/null | grep -v ^$
	set heading off ;
	set feedback off ;
...
main ()
{
	...
	then
		copyFromBackup2DB
		touch $WATIR_SERVER_HOME/Scripts/Fixture/tmp/flagToRunProgrammanager
