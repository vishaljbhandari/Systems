...
cp $WATIR_SERVER_HOME/Scripts/LTE_CDM.txt $WATIR_SERVER_HOME/data/LTE_CDM.txt
cp $WATIR_SERVER_HOME/data/LTE_CDM.txt $WATIR_SERVER_HOME/databackup/ 
...
select fieldutil.normalize(phone_number) || '|' || network_id || '|' from subscriber where status in (1,2) and subscriber_type=0;
...
replaceDatafile()
{
	...
	awk -v POSITION="$fieldposition" -v VALUE="$fieldvalue" -F ',' -f $WATIR_SERVER_HOME/Scripts/Info.awk $WATIR_SERVER_HOME/data/LTE_CDM.txt > $WATIR_SERVER_HOME/data/LTE_CDM.txt.$$
	mv $WATIR_SERVER_HOME/data/LTE_CDM.txt.$$ $WATIR_SERVER_HOME/data/LTE_CDM.txt
}
...
multipleRecords()
{
	...
	do
		cat $WATIR_SERVER_HOME/data/LTE_CDM.txt >> $WATIR_SERVER_HOME/data/LTE_CDM.txt.new
	   	i=`expr $i + 1`
	done		       	
...
do
	...
	head -$j $WATIR_SERVER_HOME/data/LTE_CDM.txt.new | tail -1 >  $WATIR_SERVER_HOME/tmpfile1
	...
	echo $FinalTimeStamp
	sed -e "s/%timestamp/$FinalTimeStamp/g" $WATIR_SERVER_HOME/tmpfile1 | sed 's/-/\//g' >> $WATIR_SERVER_HOME/databackup/LTE_CDM.txt.$$
	j=`expr $j + 1`
done
return 0
...
`echo Recordtype:1 > $WATIR_SERVER_HOME/databackup/LTERecordWithHeader.txt.$$`
cat $WATIR_SERVER_HOME/databackup/LTE_CDM.txt.$$ >> $WATIR_SERVER_HOME/databackup/LTERecordWithHeader.txt.$$
...
echo "#####################"
	sleep 1
	echo $COMMON_MOUNT_POINT/FMSData/$LTE_DATASOURCE_INPUTDIR/LTE_CDM.txt.$$
cp $WATIR_SERVER_HOME/databackup/LTE_CDM.txt.$$ $COMMON_MOUNT_POINT/FMSData/$LTE_DATASOURCE_INPUTDIR/LTE_CDM.txt.$$
return 0
...
while [ $counter -le 1000 ]
  do
        if !(test -f $COMMON_MOUNT_POINT/FMSData/$LTE_DATASOURCE_INPUTDIR/LTE_CDM.txt.$$)
        then
