        ...
        do
                ...
                do      
                        ...
                        ##      UnCompressedFile=`echo $File | sed 's/\.gz$//'`
								...
								while read i
                                do
									ls $File |grep $i 2>&1 > $RANGERHOME/tmp/tapin_cib_temp
									if [ $? == 0 ]
									then
										mv $File $Backup_Dir
										IsIgnored=1
                                ...
                                cp $File "$OutputFileDir/$File""_""$FileSufix"

                                UpdateFileDetails "$OutputFileDir/$File""_""$FileSufix"

                                touch "$OutputFileDir/success/$File""_""$FileSufix"
                                rm -f $File
                                break
                        fi
                done
