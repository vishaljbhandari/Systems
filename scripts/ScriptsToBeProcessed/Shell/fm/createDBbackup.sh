...
# in this function we fetch the sequence name and it's start value incremented by 500
getSequenceNames()
{
	cat $WATIR_SERVER_HOME/Scripts/dbcecreatebacktables.sh | grep -i "CREATE SEQUENCE" | awk {'print $4 "\n" ($10+500)'}>>$WATIR_SERVER_HOME/Scripts/sequenceNames.txt
	return 0
}
...
main ()
{
    ...
    then
        ...
        dbce_sql_file_name=$2
    else
        echo "Improper Usage: two arguments required 1:prefixForBackUpTable 2:dbce_sql_file_name"
        exit 2
    fi
	...
	bash $WATIR_SERVER_HOME/Scripts/copyData_FromTables_ToTables.sh 2 $prefixForBackUpTable
	GenerateSetupFile
}
