...
cp $RUBY_UNIT_HOME/Scripts/R01_CDM.txt $RUBY_UNIT_HOME/data/R01_CDM.txt
cp $RUBY_UNIT_HOME/data/R01_CDM.txt $RUBY_UNIT_HOME/databackup/ 
...
select name || '=>' || position from field_configs where record_config_id=2;
spool $RUBY_UNIT_HOME/data/subinfo.dat
select network_id || '|' || fieldutil.normalize(phone_number) from subscriber where status in (1,2) and Subscriber_type=0;
...
generateOneRecord()
{
		...
		databasearguments=`echo $databasenetwork`
		k=1	
...
replaceDatafile()
{
	...
	awk -v POSITION="$fieldposition" -v VALUE="$fieldvalue" -F ',' -f $RUBY_UNIT_HOME/Scripts/Info.awk $RUBY_UNIT_HOME/data/R01_CDM.txt > $RUBY_UNIT_HOME/data/Recharge.txt.$$
	mv $RUBY_UNIT_HOME/data/Recharge.txt.$$ $RUBY_UNIT_HOME/data/R01_CDM.txt
}
...
multipleRecords()
{
	...
	do
		cat $RUBY_UNIT_HOME/data/R01_CDM.txt >> $RUBY_UNIT_HOME/data/R01_CDM.txt.new
	   	i=`expr $i + 1`
	done		       	
}
...
do
	...
	head -$j $RUBY_UNIT_HOME/data/R01_CDM.txt.new | tail -1 >  $RUBY_UNIT_HOME/tmpfile1
	...
	echo $FinalTimeStamp
	sed -e "s/%timestamp/$FinalTimeStamp/g" $RUBY_UNIT_HOME/tmpfile1 | sed 's/-/\//g' >> $RUBY_UNIT_HOME/databackup/RechargeRecord.txt.$$
	j=`expr $j + 1`
done
...
{
sleep 1
cp $RUBY_UNIT_HOME/databackup/RechargeRecord.txt.$$ $RANGERHOME/FMSData/$RECHARGELOG_DATASOURCE_INPUTDIR/RechargeRecord.txt.$$
...
while [ $counter -le 1000 ]
  do
        if !(test -f $RANGERHOME/FMSData/$RECHARGELOG_DATASOURCE_INPUTDIR/RechargeRecord.txt.$$)
        then
