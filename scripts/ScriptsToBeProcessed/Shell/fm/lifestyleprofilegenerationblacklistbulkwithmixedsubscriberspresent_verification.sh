...
select 1 from dual where 3 = (select NVL(count(*), 0) from subscriber where subscriber_type = $FRAUD_SUBSCRIBER); 
select 1 from dual where 3 = (select NVL(count(*), 0) from subscriber where subscriber_type = $ACTIVE_SUBSCRIBER and id > 1024); 
...
select 1 from dual where 1 = (select count(*) from audit_trails
				 where source = (select id from source where description = 'Hot List Management')
					and event_code = 7
...
select 1 from dual where 1 = (select count(*) from configurations where config_key = 'ACA_LAST_PROCESSED_LSP_BLK_ID_ST_1_PT_1_1024' and value = '66');

--select 1 from dual where 'TRUE' = (select value from configurations where config_key = 'ACA_STOP_PROCESSING_ST_1_PT_1_1024') ;

select 1 from dual where 6 = (select count(*) from blacklist_profile_options where network_id = 1024 and PHONE_NUMBER = '+000000000') ;
