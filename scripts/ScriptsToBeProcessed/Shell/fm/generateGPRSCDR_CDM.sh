...
cp $RUBY_UNIT_HOME/Scripts/G01_CDM.txt $RUBY_UNIT_HOME/data/G01_CDM.txt
cp $RUBY_UNIT_HOME/data/G01_CDM.txt $RUBY_UNIT_HOME/databackup/ 
...
if [ $IS_RATING_ENABLED = "1" ]
		then 
rm -f $RANGERHOME/$RATER_GPRS_INPUTDIR/*.*
rm -f $RANGERHOME/$RATER_GPRS_INPUTDIR/success/*.*
rm -f $RANGERHOME/$RATER_GPRS_OUTPUTDIR/*.*
rm -f $RANGERHOME/$RATER_GPRS_OUTPUTDIR/success/*.*
fi
return 0
}
...
select name || '=>' || position from field_configs where record_config_id=7;
...
sed -e "s/Value (INR)/Value/g" $RUBY_UNIT_HOME/data/ds_field_config_gprs.dat > $RUBY_UNIT_HOME/data/Final_ds_field_config_gprs.dat
...
replaceDatafile()
{
	...
	awk -v POSITION="$fieldposition" -v VALUE="$fieldvalue" -F ',' -f $RUBY_UNIT_HOME/Scripts/Info.awk $RUBY_UNIT_HOME/data/G01_CDM.txt > $RUBY_UNIT_HOME/data/G01_CDM.txt.$$
	mv $RUBY_UNIT_HOME/data/G01_CDM.txt.$$ $RUBY_UNIT_HOME/data/G01_CDM.txt
}
...
multipleRecords()
{
	...
	do
		cat $RUBY_UNIT_HOME/data/G01_CDM.txt >> $RUBY_UNIT_HOME/data/G01_CDM.txt.new
	   	i=`expr $i + 1`
	done		       	
...
do
	...
	head -$j $RUBY_UNIT_HOME/data/G01_CDM.txt.new | tail -1 >  $RUBY_UNIT_HOME/tmpfile1
...
{
sleep 1
cp $RUBY_UNIT_HOME/databackup/CDRRecord.txt.$$ $RANGERHOME/FMSData/$GPRS_DATASOURCE_INPUTDIR/GPRSCDRRecord.txt.$$
if [ $IS_RATING_ENABLED = "1" ]
		then 
echo "Copied file to DataSource"
	while [ ! -f $RANGERHOME/FMSData/$RATER_GPRS_INPUTDIR/GPRSCDRRecord.txt.$$ ]
		do
			sleep 0.2
		done
	cp $RANGERHOME/FMSData/$RATER_GPRS_INPUTDIR/GPRSCDRRecord.txt.$$ $RANGERHOME/FMSData/$RATER_GPRS_INPUTDIR/success/
	 while [ ! -f $RANGERHOME/FMSData/$RATER_GPRS_OUTPUTDIR/GPRSCDRRecord.txt.$$ ]
		do
			sleep 0.2
		done
        cp $RANGERHOME/FMSData/$RATER_GPRS_OUTPUTDIR/GPRSCDRRecord.txt.$$ $RANGERHOME/FMSData/DataRecord/Process/
fi
return 0
}
...
while [ $counter -le 1000 ]
  do
 	if !(test -f $RANGERHOME/FMSData/$GPRS_DATASOURCE_INPUTDIR/GPRSCDRRecord.txt.$$)
        then
