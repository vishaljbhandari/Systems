...
set heading off
spool lifestyleprofilegenerationactivesubscriberV.dat

select 1 from dual where 1 = (select NVL(count(*), 0) from subscriber where phone_number='+919845200005');
select 1 from dual where 1 = (select NVL(count(*), 0) from subscriber where phone_number='+919845200006');
select 1 from dual where 1 = (select count(*) from account where id = 1025) ;
select 1 from dual where 1 = (select count(*) from account where id = 1026) ;
select 1 from dual where 1 = (select count(*) from subscriber where subscriber_type = $ACTIVE_SUBSCRIBER and id = 5200); 
select 1 from dual where 1 = (select count(*) from subscriber where subscriber_type = $ACTIVE_SUBSCRIBER and id = 5201); 
...
select 1 from dual where 1 = (select NVL(count(*), 0) from subscriber 
									where phone_number='+919845200006'
										and subscriber_type = $ACTIVE_SUBSCRIBER);
...
select 1 from dual where 1 = (select count(*) from usage_profiles
				where record_type = 2
					and call_type = 1
					and subscriber_id = 5201
					and phone_number = '+919845200006'                             
					and subscriber_type = $ACTIVE_SUBSCRIBER
					and cdr_count = 1
					and avg_duration = 40                             
					and avg_value = 10); 
...
select 1 from dual where 1 = (select count(*) from suspect_values
				 where value = '404212000005'
					and source = 'Account Name : A.0000001040 Phone Number : +919845200005'
					and reason = 'FCN'
					and user_id = 2 
...
select 1 from dual where 1 = (select count(*) from audit_trails
				 where source = 7
					...
					and entity_value = '404212000005'
					and action = 'Hotlist Entity IMEI ''404212000005'' is added'
					and user_id = 'system'
					and network_id = '-');
...
select 1 from dual where 1 = (select count(*) from hotlist_groups_suspect_values 
				 where hotlist_group_id = (select id from hotlist_groups where name = 'Fraudulent') 
					and suspect_value_id = (select id from suspect_values where value = '404212000005'));

select 1 from dual where 1 = (select count(*) from suspect_values
				where value = '504220000005'
					and source = 'Account Name : A.0000001040 Phone Number : +919845200005'
					and reason = 'FCN'
					and user_id = 2
...
select 1 from dual where 1 = (select count(*) from audit_trails
				 where source = 7
					and event_code = 7
					and entity_type = 4000 
					and entity_value = '504220000005'
					and action = 'Hotlist Entity IMSI ''504220000005'' is added'
					and user_id = 'system'
					and network_id = '-');
					 
select  1 from dual where 1 = (select count(*) from hotlist_groups_suspect_values
				 where hotlist_group_id = (select id from hotlist_groups where name = 'Fraudulent') 
					and suspect_value_id = (select id from suspect_values where value = '504220000005'));
...
select  1 from dual where 1 = (select count(*) from configurations where
									config_key='ACA_LAST_PROCESSED_LSP_BLK_ID_ST_0_PT_1_1024' and value='12');

spool off
EOF

if [ $? -eq '5' ] || grep "no rows" lifestyleprofilegenerationactivesubscriberV.dat
then
	exitval=1
else
	exitval=0
fi
rm -f lifestyleprofilegenerationactivesubscriberV.dat
exit $exitval
