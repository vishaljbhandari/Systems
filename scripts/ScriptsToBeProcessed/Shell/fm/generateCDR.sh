...
cp $RUBY_UNIT_HOME/Scripts/C01.txt $RUBY_UNIT_HOME/data/C01.txt
cp $RUBY_UNIT_HOME/data/C01.txt $RUBY_UNIT_HOME/databackup/ 
...
sed -e "s/IMSI/IMSI/g" $RUBY_UNIT_HOME/data/ds_field_config.dat > $RUBY_UNIT_HOME/data/ds_field_config_tmp.dat
sed -e "s/IMEI/EquipmentID/g" $RUBY_UNIT_HOME/data/ds_field_config_tmp.dat > $RUBY_UNIT_HOME/data/Final_ds_field_config_tmp.dat
sed -e "s/Value (INR)/Value/g" $RUBY_UNIT_HOME/data/Final_ds_field_config_tmp.dat > $RUBY_UNIT_HOME/data/Final_ds_field_config.dat
...
replaceDatafile()
{
	...
	awk -v POSITION="$fieldposition" -v VALUE="$fieldvalue" -F ',' -f $RUBY_UNIT_HOME/Scripts/Info.awk $RUBY_UNIT_HOME/data/C01.txt > $RUBY_UNIT_HOME/data/C01.txt.$$
	mv $RUBY_UNIT_HOME/data/C01.txt.$$ $RUBY_UNIT_HOME/data/C01.txt
}
...
multipleRecords()
{
	...
	do
		cat $RUBY_UNIT_HOME/data/C01.txt >> $RUBY_UNIT_HOME/data/C01.txt.new
	   	i=`expr $i + 1`
	done		       	
...
do
	...
	head -$j $RUBY_UNIT_HOME/data/C01.txt.new | tail -1 >  $RUBY_UNIT_HOME/tmpfile1
...
cp $RUBY_UNIT_HOME/databackup/CDRRecordWithHeader.txt.$$ $RANGERHOME/FMSData/DataRecord/Process/CDRRecord.txt.$$
cp $RUBY_UNIT_HOME/databackup/CDRRecordWithHeader.txt.$$ $RANGERHOME/FMSData/DataRecord/Process/success/CDRRecord.txt.$$
mv $RUBY_UNIT_HOME/databackup/CDRRecordWithHeader.txt.$$ $RUBY_UNIT_HOME/database/CDRRecord.txt.$$
...
while [ $counter -le 200 ]
  do
        if !(test -f $RANGERHOME/FMSData/DataRecord/Process/CDRRecord.txt.$$)
        then
              break
         else
               sleep 0.1
