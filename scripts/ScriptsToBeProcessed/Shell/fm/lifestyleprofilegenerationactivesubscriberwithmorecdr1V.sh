...
set heading off
spool lifestyleprofilegenerationactivesubscriberwithmorecdr1V.dat
...
select  1 from dual where 1 = (select count(*) from gprs_life_style_profiles
				where  phone_number = '+919845200005'
					...
					and  day_count = 2
					and  total_value = 140
					and  total_upload_volume = 1000
					and  total_download_volume = 600);
...
if [ $? -eq '5' ] || grep "no rows" lifestyleprofilegenerationactivesubscriberwithmorecdr1V.dat
then
	exitval=1
else
	exitval=0
fi
rm -f lifestyleprofilegenerationactivesubscriberwithmorecdr1V.dat
exit $exitval
