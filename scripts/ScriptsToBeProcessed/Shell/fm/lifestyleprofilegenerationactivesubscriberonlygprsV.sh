...
set heading off
spool lifestyleprofilegenerationactivesubscriberonlygprsV.dat
...
select 1 from dual where 1 = (select count(*) from subscriber where subscriber_type = $ACTIVE_SUBSCRIBER and id = 1025); 

select 1 from dual where 1 = (select NVL(count(*), 0) from subscriber 
									where phone_number='+919812400401'
...
if [ $? -eq '5' ] || grep "no rows" lifestyleprofilegenerationactivesubscriberonlygprsV.dat
then
	exitval=1
else
	exitval=0
fi
rm -f lifestyleprofilegenerationactivesubscriberonlygprsV.dat
exit $exitval
