...
FULL_FEATURE_LIST="LICENSE LDAP STANDALONE PREVEASUPPORT REJECT_FUTURE_RECORDS DASHBOARD GSM GPRS RECHARGE CDMA AI AUTOACCOUNTCLOSE MLH NRTRDE ISP ACCOUNT_CANCEL_ON_LAST_SUBSCRIBER INTERNAL_USER FINGER_PRINT_PROFILE PRECHECK BITMAP I2 INLINEMODULE VOICE RTF PARTITIONED_DBSETUP DB2 ORACLE"
...
clear;
echo -e "Nikira V7 Setup Script \n";
GetInput "Is this a full Nikira Installation?"
isFull=$?
...
then
	...
	if [ $selectedModule -eq 1 ]
	then
		FEATURE_LIST="LDAP STANDALONE REJECT_FUTURE_RECORDS DASHBOARD GSM GPRS RECHARGE CDMA AUTOACCOUNTCLOSE ISP ACCOUNT_CANCEL_ON_LAST_SUBSCRIBER"
		echo -e "Select the features for the FP Module.....\n";
	fi

	if [ $selectedModule -eq 2 ]
	then
		FEATURE_LIST="LDAP STANDALONE DASHBOARD"
		echo -e "Select the features for the Internal Affairs Module.....\n";
	fi

	if [ $selectedModule -eq 3 ]
	then
		FEATURE_LIST="LDAP STANDALONE DASHBOARD AUTOACCOUNTCLOSE ACCOUNT_CANCEL_ON_LAST_SUBSCRIBER"
		echo -e "Select the features for the Alias Module.....\n";
	fi
fi
