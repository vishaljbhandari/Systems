...
TMP_FILE_PATH=$CURRENT_DIR/tmp/start_servers.tmp
DASHBOARD_LOG=$CURRENT_DIR/log/dashboard.log
>$DASHBOARD_LOG
...
NEWNOTIFICATIONMANAGER_PID_FILE=$CURRENT_DIR/log/newnotificationmanager.pid
QUERYMANAGER_PID_FILE=$CURRENT_DIR/log/querymanager.pid
NOTIFICATIONMANAGER_DIR=$CURRENT_DIR/../notificationmanager
NEWNOTIFICATIONMANAGER_DIR=$NMROOT/bin
...
get_newnotificationmanager_status()
{
	if [ -f "$NEWNOTIFICATIONMANAGER_PID_FILE" ]
	then
		
		NEWNOTIFICATIONMANAGER_PID=`cat $NEWNOTIFICATIONMANAGER_PID_FILE`
		if [ "`ps -u$LOGNAME | grep $NEWNOTIFICATIONMANAGER_PID`x" == "x" ]
		then
			NEWNOTIFICATIONMANAGER_STATUS="n"
		else
			NEWNOTIFICATIONMANAGER_STATUS="y"
		fi
	else
		NEWNOTIFICATIONMANAGER_STATUS="n"
	fi
...
start_notificationmanager()
{
	...
	then
		...
		ruby $NOTIFICATIONMANAGER_DIR/server.rb >> $TMP_FILE_PATH 2>&1 &
...
start_newnotificationmanager()
{
	put_starting_message "Starting New NotificationManager:"
	get_newnotificationmanager_status
	if [ $NEWNOTIFICATIONMANAGER_STATUS == "y" ]
	then
		ruby -e "puts \"$ALREADY_RUNNING\""
	else    
		cur_dir=`pwd`
		cd $NEWNOTIFICATIONMANAGER_DIR
		$NEWNOTIFICATIONMANAGER_DIR/notificationmanager -f $NEWNOTIFICATIONMANAGER_DIR/nm_config.xml >> $cur_dir/tmp/nm.tmp 2>&1 &
		cd $cur_dir	
		newnotificationmanager_pid=$!
		echo $newnotificationmanager_pid > $NEWNOTIFICATIONMANAGER_PID_FILE
		sleep 2
		get_newnotificationmanager_status
		if [ $NEWNOTIFICATIONMANAGER_STATUS == "y" ]	
		then
...
stop_newnotificationmanager()
{
	put_starting_message "Stopping New NotificationManager:"
	get_newnotificationmanager_status
	if [ $NEWNOTIFICATIONMANAGER_STATUS == "n" ]
	then
		ruby -e "puts \"$NOT_RUNNING\""
	else
		wait_and_kill_pid $NEWNOTIFICATIONMANAGER_PID

		get_newnotificationmanager_status
		if [ $NEWNOTIFICATIONMANAGER_STATUS == "n" ]	
		then
			ruby -e "puts \"$STOPPED_MESSAGE\""
			rm -rf $NEWNOTIFICATIONMANAGER_PID_FILE
		else
			ruby -e "puts \"$FAILED\""
		fi
	fi
...
start_juggernaut()
{
	...
	then
		...
		juggernaut -c $CURRENT_DIR/juggernaut.yml -l $CURRENT_DIR/log/juggernaut.log -P $CURRENT_DIR/log/juggernaut.pid -e -d  >> $TMP_FILE_PATH 2>&1
		sleep 2
		get_juggernaut_status
		if [ $JUGGERNAUT_STATUS == "y" ]	
		then
...
reload_newnotificationmanager()
{
	get_newnotificationmanager_status
	if [ "$NEWNOTIFICATIONMANAGER_STATUS" == "y" ]
	then
		put_starting_message "Reloading New NotificationManager:"
		kill -HUP `cat $NEWNOTIFICATIONMANAGER_PID_FILE`
		ruby -e "puts \"$RELOADED\""	
	else
		print_status newnotificationmanager
	fi
}
...
`tput cup ${_OPTION_PREV_ROW} 16`
    ...
    ${_BOLD}Q${_NORM}) Exit
		${_FULL_UNDERLINE}
    Also you can use : $0 {start|stop|restart|status|clearcache|start_without_dashboard|start_notificationmanager||start_newnotificationmanager|stop_newnotificationmanager|reload_newnotificationmanager}
    Ex: $0 status
		${_FULL_UNDERLINE}
	EOF
    ...
    case ${_ANS} in
      ...
      [Cc])  
          ...
          print_status Juggernaut Memcached Querymanager Notificationmanager NewNotificationmanager $APACHE_PARAM $DASHBOARD_PARAM
        ;;
      [Dd])  
          stop_all
          start_all
...
case $ARGV in
	...
	status)
		...
		print_status Juggernaut Memcached $QM_PARAM Notificationmanager NewNotificationManager $APACHE_PARAM $DASHBOARD_PARAM
	;;
esac	
