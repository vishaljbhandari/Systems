...
InputFile=$1
LogFile=`pwd`/$InputFile.checker.log
...
printf "Number of files for to check are $countofrecords\n" >> $LogFile
echo -e "Number of files for to check are $countofrecords"

while read line
do
	if [ -f  $ROOTDIR/$line$extToken ]
	then
     		echo -e "File $ROOTDIR/$line$extToken -  Is Present"
		printf "File $ROOTDIR/$line$extToken - Is Present\n" >> $LogFile
	else
        	printf "File : $ROOTDIR/$line$extToken ---------------- Not Present\n" >> $LogFile
        	echo -e "File : $ROOTDIR/$line$extToken ---------------- Not Present"
	fi
