...
if [ ! -f $InputFile ]
then
        printf "File Does not Exist: $InputFile \n-----------------\n" >> $LogFile
        echo -e "File Does not Exist: $InputFile"
	echo -e "TERMINATED : Please check Log File at $LogFile"
        exit 3
else
...
do
	...
	then
		cp $ROOTDIR/$line$FirstSuffix $ROOTDIR/$line$SecondSuffix
     		echo -e "File $line - $ActionType Done"
		printf "File $line - $ActionType Done \n" >> $LogFile
	else
        	printf "File Doesnt Exist: $ROOTDIR/$line ---------------- SKIPPED\n" >> $LogFile
        	echo -e "File Doesnt Exist: $ROOTDIR/$line ---------------- SKIPPED"
	fi
