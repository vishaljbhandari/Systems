...
if [ ! -f $ERROR_DB_FILE ]; then
	...
	cat $CASES_DIR/".statMasterHeader" > $ERROR_DB_FILE
fi
...
do	
	...
	echo -e $STAT_DATA_FILE_TIMESTAMP"##"$ERR_ERROR_ID"##"$ERR_FILE_NAME"##"$ERR_FILE_PATH"##"$ERR_PRIORITY"##"$ERR_CATEGORY"##"$ERR_LINE_NUMBER"##"$ERR_ERROR_VERSION"##"$ERR_FIVE_LINE_HASH"##"$ERR_FILE_HASH"##"$ERR_PROJECT"##"$ERR_MODULE"##"$ERR_LINK >> $CASE_DIR/.tmpStater
...
#01 Date		=> Rundate
#02 ErrorId		=> cksum of FileName, 
#03 FileName	=> 1st token of line, sep by ##
#04 FilePath	=> 5th token of line, sep by ##
#05 Priority	=> 2nd token of line, sep by ##
#06 Category	=> 4th token of line, sep by ##
#07 LineNumber	=> 3rd token of line, sep by ##
#08 ErrorVersion	=> 
#09 3LineErrorHashCode	=> LineNumber-1 to LineNumber+1 Lines of error file and get hash
