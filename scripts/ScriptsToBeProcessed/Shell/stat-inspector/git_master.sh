...
git log --pretty=format:"%h#-#%H#-#%an#-#%ae#-#%cd#-#%ct#-#%s" --since="$GIT_STARTING_DATE" --date=format:'%Y%m%d' --reverse > $LANDING_DIR/$GIT_SUMMARY_REPORT_PREFIX$GCURRENTTIME
echo -e "[ INFO] Preparing detailed report from "$GROOT_DIRECTORY"/src"
git log --stat --since="$GIT_STARTING_DATE" --reverse > $LANDING_DIR/$GIT_DETAILED_FILE_PREFIX$GCURRENTTIME
